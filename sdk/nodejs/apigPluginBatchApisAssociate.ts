// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

export class ApigPluginBatchApisAssociate extends pulumi.CustomResource {
    /**
     * Get an existing ApigPluginBatchApisAssociate resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: ApigPluginBatchApisAssociateState, opts?: pulumi.CustomResourceOptions): ApigPluginBatchApisAssociate {
        return new ApigPluginBatchApisAssociate(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'sbercloud:index/apigPluginBatchApisAssociate:ApigPluginBatchApisAssociate';

    /**
     * Returns true if the given object is an instance of ApigPluginBatchApisAssociate.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is ApigPluginBatchApisAssociate {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === ApigPluginBatchApisAssociate.__pulumiType;
    }

    /**
     * The list of API IDs to be bound by the plugin.
     */
    declare public readonly apiIds: pulumi.Output<string[]>;
    /**
     * The script configuration value of this change is also the original value used for comparison with
     * the new value next time the change is made. The corresponding parameter name is 'api_ids'.
     */
    declare public readonly apiIdsOrigins: pulumi.Output<string[]>;
    /**
     * The environment ID where the API was published.
     */
    declare public readonly envId: pulumi.Output<string>;
    /**
     * The ID of the dedicated instance to which the plugin belongs.
     */
    declare public readonly instanceId: pulumi.Output<string>;
    /**
     * The plugin ID.
     */
    declare public readonly pluginId: pulumi.Output<string>;
    /**
     * The region where the plugin is located.
     */
    declare public readonly region: pulumi.Output<string>;

    /**
     * Create a ApigPluginBatchApisAssociate resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: ApigPluginBatchApisAssociateArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: ApigPluginBatchApisAssociateArgs | ApigPluginBatchApisAssociateState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as ApigPluginBatchApisAssociateState | undefined;
            resourceInputs["apiIds"] = state?.apiIds;
            resourceInputs["apiIdsOrigins"] = state?.apiIdsOrigins;
            resourceInputs["envId"] = state?.envId;
            resourceInputs["instanceId"] = state?.instanceId;
            resourceInputs["pluginId"] = state?.pluginId;
            resourceInputs["region"] = state?.region;
        } else {
            const args = argsOrState as ApigPluginBatchApisAssociateArgs | undefined;
            if (args?.apiIds === undefined && !opts.urn) {
                throw new Error("Missing required property 'apiIds'");
            }
            if (args?.envId === undefined && !opts.urn) {
                throw new Error("Missing required property 'envId'");
            }
            if (args?.instanceId === undefined && !opts.urn) {
                throw new Error("Missing required property 'instanceId'");
            }
            if (args?.pluginId === undefined && !opts.urn) {
                throw new Error("Missing required property 'pluginId'");
            }
            resourceInputs["apiIds"] = args?.apiIds;
            resourceInputs["apiIdsOrigins"] = args?.apiIdsOrigins;
            resourceInputs["envId"] = args?.envId;
            resourceInputs["instanceId"] = args?.instanceId;
            resourceInputs["pluginId"] = args?.pluginId;
            resourceInputs["region"] = args?.region;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(ApigPluginBatchApisAssociate.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * Input properties used for looking up and filtering ApigPluginBatchApisAssociate resources.
 */
export interface ApigPluginBatchApisAssociateState {
    /**
     * The list of API IDs to be bound by the plugin.
     */
    apiIds?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * The script configuration value of this change is also the original value used for comparison with
     * the new value next time the change is made. The corresponding parameter name is 'api_ids'.
     */
    apiIdsOrigins?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * The environment ID where the API was published.
     */
    envId?: pulumi.Input<string>;
    /**
     * The ID of the dedicated instance to which the plugin belongs.
     */
    instanceId?: pulumi.Input<string>;
    /**
     * The plugin ID.
     */
    pluginId?: pulumi.Input<string>;
    /**
     * The region where the plugin is located.
     */
    region?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a ApigPluginBatchApisAssociate resource.
 */
export interface ApigPluginBatchApisAssociateArgs {
    /**
     * The list of API IDs to be bound by the plugin.
     */
    apiIds: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * The script configuration value of this change is also the original value used for comparison with
     * the new value next time the change is made. The corresponding parameter name is 'api_ids'.
     */
    apiIdsOrigins?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * The environment ID where the API was published.
     */
    envId: pulumi.Input<string>;
    /**
     * The ID of the dedicated instance to which the plugin belongs.
     */
    instanceId: pulumi.Input<string>;
    /**
     * The plugin ID.
     */
    pluginId: pulumi.Input<string>;
    /**
     * The region where the plugin is located.
     */
    region?: pulumi.Input<string>;
}
